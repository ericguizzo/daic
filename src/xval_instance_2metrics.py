...
def run_experiment(num_experiment, num_run, num_folds, dataset, experiment_folder, parameters, gpu_ID, task_type, generator):
    ...
    for i in range(num_folds):
        ...
        results_name = output_temp_results_path + '/temp_results_' + dataset + '_exp' + str(num_experiment) + '_run' + str(num_run) + '_fold' + str(num_fold)
        ...
        if BACKEND == 'torch':
            training = subprocess.Popen(['python3', 'training_torch_2metrics.py',
                                         'crossvalidation', str(num_experiment), str(num_run),
                                          str(num_fold), parameters, model_name, results_name,
                                          output_temp_data_path, dataset, str(gpu_ID),
                                          str(num_folds), locals()['task_type'],parameters_name,
    ...
    for curr_metric in range(2):
        ...
        #save results dict
        if curr_metric == 0:
            dict_name = 'results_BVL_' + dataset + '_exp' + str(num_experiment) + '_run' + str(num_run) + '.npy'
        ...
        if curr_metric == 1:
            dict_name = 'results_BVA_' + dataset + '_exp' + str(num_experiment) + '_run' + str(num_run) + '.npy'
