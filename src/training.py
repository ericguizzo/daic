...
def main():
    ...
    if generator:
        ...
        train_pred = best_model.predict_generator(training_generator, steps=len(training_target)/batch_size+1)
        val_pred = best_model.predict_generator(validation_generator, steps=len(validation_target)/batch_size+1)
        test_pred = best_model.predict_generator(test_generator, steps=len(test_target)/batch_size+1)
    else:
